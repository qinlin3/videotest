	...
	'Login':{
		...
		'Create':function($id){
			...
			if(MAC.Cookie.Get('userid')!=undefined && MAC.Cookie.Get('userid')!=''){ 
				...
				html+='<form id="loginform" onsubmit="return false;" action="'+SitePath+'index.php?user-check" method="post"><div class="username clearfix"><div class="yhm">账户</div><div class="srk"><input name="username" type="text" class="input" id="username"/></div></div><div class="userpass clearfix"><div class="yhmm">密码</div><div class="srk"><input name="userpass" type="password" class="input" id="userpass"/></div></div><div class="idl clearfix"><div class="idl-l"><input class="formbutton" id="loginbtn" type="submit" value="登 录"></div><div class="idl-r"><a class="qq-login" href="'+SitePath+'index.php?m=user-reg-ref-qqlogin.html"></a></div></div><div class="idl2 clearfix"><a title="忘记密码" class="forgotpass" href="'+SitePath+'index.php?m=user-findpass.html">忘记密码?</a><a class="reg-btn" href="'+SitePath+'index.php?m=user-reg.html" target="_blank">还没有账号?</a></div></form>';
			}
			...
			if(jsondata.length > 0){
				for($i=0; $i<jsondata.length; $i++){
					if($i%2==1){
						...
						html +='<dd class="even">';
					}//innerStr += "<li><h5><a href=\"" + urlArray[i] + "\">"+nameArray[i]+"</a></h5><label><a class=\"color\" href=\"" + urlArray[i] + "\">继续观看</a></label><span class=\"del\"><a href=\"javascript:void(0);\" class='delck' data='"+i+"' title='删除' mtype='inck'>删除</a></span></li>"	
	...
	'Suggest': {
		'Show': function($id,$limit,$ajaxurl,$jumpurl){
			...
			$("#"+$id).autocomplete($ajaxurl,{
				width: 413,scrollHeight: 300,minChars: 1,matchSubset: 1,max: $limit,cacheLength: 10,multiple: true,matchContains: true,autoFill: false,dataType: "json",
				parse:function(obj) {
					if(obj.status){
						var parsed = [];
